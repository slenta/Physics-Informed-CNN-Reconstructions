...
def correlation_plotting(path, iteration, depth):
    ...
    f_a = h5py.File(
        f"../Asi_maskiert/results/validation/{path}/validation_{iteration}_assimilation_{cfg.mask_argo}_{cfg.eval_im_year}.hdf5",
        "r",
    )
    f_o = h5py.File(
        f"../Asi_maskiert/results/validation/{path}/validation_{iteration}_observations_{cfg.mask_argo}_{cfg.eval_im_year}.hdf5",
        "r",
    )
    f_ah = h5py.File(
        f"../Asi_maskiert/results/validation/{path}/heatcontent_{iteration}_assimilation_{cfg.mask_argo}_{cfg.eval_im_year}.hdf5",
        "r",
    )
    f_oh = h5py.File(
        f"../Asi_maskiert/results/validation/{path}/heatcontent_{iteration}_observations_{cfg.mask_argo}_{cfg.eval_im_year}.hdf5",
        "r",
    )
    ...
    acc_mean_argo_o = pearsonr(
        np.nanmean(np.nanmean(hc_gt[552:], axis=1), axis=1),
        np.nanmean(np.nanmean(hc_o[552:], axis=1), axis=1),
    )[0]
    acc_mean_preargo_o = pearsonr(
        np.nanmean(np.nanmean(hc_gt[:552], axis=1), axis=1),
        np.nanmean(np.nanmean(hc_o[:552], axis=1), axis=1),
    )[0]
...
def timeseries_plotting(path, iteration, obs=True, del_t=1):
    ...
    std_o = evalu.running_mean_std(
        np.nanstd(hc_all_o, axis=0), mode="mean", del_t=del_t
    )
    ...
    if obs == True:
        ...
        plt.plot(hc_obs, label="Directly Reconstructed Observations", color="darkgreen")
        plt.fill_between(
            range(len(hc_gt)),
            hc_gt + del_gt,
            hc_gt - del_gt,
