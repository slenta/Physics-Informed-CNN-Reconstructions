...
class preprocessing():
    ...
    def save_data(self):
        ...
        if cfg.lstm_steps:
            ...
            if self.mode=='val'
                f = h5py.File(f'{self.path}{self.name}{self.year}_{self.attributes[0]}_{self.attributes[1]}_{self.attributes[2]}_{str(cfg.in_channels)}_observations.hdf5', 'w')
            elif self.mode == 'mixed':
                f = h5py.File(f'{self.path}Mixed_{self.year}_{self.attributes[0]}_{self.attributes[1]}_{self.attributes[2]}_{str(cfg.in_channels)}.hdf5', 'w')
            elif self.mode == 'mask':
                f = h5py.File(f'{self.path}{self.name}{self.year}_{self.attributes[0]}_{self.attributes[1]}_{self.attributes[2]}_{str(cfg.in_channels)}_{str(cfg.ensemble_member)}.hdf5', 'w')
            else:
                f = h5py.File(f'{self.path}{self.name}{self.year}_{self.attributes[0]}_{self.attributes[1]}_{self.attributes[2]}_{str(cfg.in_channels)}.hdf5', 'w')
